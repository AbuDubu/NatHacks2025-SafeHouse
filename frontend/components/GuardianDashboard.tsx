      ...
      const residentsData: Resident[] = await Promise.all(
        usersToLoad.map(async (user) => {
          try {
            ...
            // Use sleep from readings (synced weekly average from Apple Health)
            // Guardian sees the same weekly average sleep as resident
            const sleepReading = readings.find(r => matchesSensorType(r.sensor?.sensor_type, 'sleep'));
            const sleep = sleepReading ? sleepReading.value : (sleepHours > 0 ? sleepHours : 0);
            
            // Use steps from readings (synced from Apple Health)
            // Guardian sees the same step count as resident
            const activityReading = readings.find(r => 
              matchesSensorType(r.sensor?.sensor_type, 'step_count') || 
              matchesSensorType(r.sensor?.sensor_type, 'activity')
            );
            const activity = activityReading ? Math.round(activityReading.value) : (steps > 0 ? Math.round(steps) : 0);
            ...
            return {
              ...
              vitals: {
                ...
                glucose: bloodGlucose > 0 ? Math.round(bloodGlucose) : 0,
                  ...
                  <View key={resident.id} style={[styles.residentCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
                    ...
                    <View style={styles.vitalsGrid}>
                      ...
                      <View style={[styles.vitalItem, { backgroundColor: colors.muted }]}>
                        ...
                        <Text style={[styles.vitalValue, { color: colors.foreground }]}>
                          {resident.vitals.sleep > 0 ? `${resident.vitals.sleep.toFixed(1)}h avg` : '--'}
                        </Text>
                      </View>
                      <View style={[styles.vitalItem, { backgroundColor: colors.muted }]}>
                        <Text style={[styles.vitalLabel, { color: colors.mutedForeground }]}>Activity</Text>
                        <Text style={[styles.vitalValue, { color: colors.foreground }]}>
                          {resident.vitals.activity > 0 ? `${resident.vitals.activity.toLocaleString()} steps` : '--'}
                        </Text>
                      </View>
                    </View>
